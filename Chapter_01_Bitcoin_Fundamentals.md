...
5. **nBits** (4 bytes): A compact representation of the target threshold that the block's hash must fall below. This encodes the current difficulty level.
...
Bitcoin emerged as a direct response to the 2008 financial crisis, embedding its critique of centralized banking into its very first block. At its core, Bitcoin solves the fundamental challenge of digital money: enabling thousands of computers worldwide to agree on ownership without requiring a trusted third party. This breakthrough relies on Nakamoto Consensus, where nodes follow the chain with the most accumulated proof of work, and miners compete in an energy-intensive cryptographic lottery to earn the right to add new blocks every ten minutes on average.

The system's technical architecture centers on the UTXO model, which tracks ownership through individual "coins" rather than account balances, preventing double-spending while enabling sophisticated programmability through Bitcoin Script. Bitcoin's monetary policy operates with mathematical precision: a fixed supply cap of 21 million coins enforced through halvings every four years, creating predictable scarcity that stands in stark contrast to fiat currencies. This deflationary design faces a critical long-term challenge as block subsidies approach zero around 2140, requiring transaction fees to sustain the network's security budget.

Protocol evolution occurs through soft forks, backward-compatible upgrades that tighten consensus rules without splitting the network. The SegWit activation of 2017 demonstrated Bitcoin's antifragile governance, where economic nodes successfully coordinated to overcome miner resistance through the credible threat of a User Activated Soft Fork. Subsequent upgrades like Taproot have enhanced privacy and programmability, while Layer 2 solutions like the Lightning attempt to address the base layer limitations for small payments, though significant liquidity and adoption challenges remain.

Scaling Bitcoin beyond its base layer settlement capacity remains contentious. True L2s would require covenant opcodes like OP_CAT, CTV or CSFS to enable cryptographic enforcement of withdrawals, capabilities that remain locked behind the slow, deliberate process of soft fork activation. This tension between conservative protocol ossification and demands for expanded functionality defines much of Bitcoin's current development discourse.

Bitcoin's security model distributes power across distinct roles: miners control transaction inclusion and ordering but cannot override consensus rules enforced by full nodes, while markets ultimately determine value through the collective choices of users, exchanges, and merchants. This creates a system designed to be antifragile, growing stronger from stress and attacks. Recent innovations like Ordinals have demonstrated how Bitcoin's base layer can support new use cases through creative application of existing features, enabling digital artifacts and experimental token systems while maintaining the network's fundamental properties of decentralization and censorship resistance.